...
public class CreditCardOperations extends Operations
{
    ...
    {
       System.out.println("Balance USD "+Bank.creditBalanceList.get(cardNumber));
    }
    public void deposit(int cardNumber,double depositAmount)
    {
        Bank.creditBalanceList.put(cardNumber,Bank.creditBalanceList.get(cardNumber)+depositAmount);
    ...
    public void withdraw(int cardNumber,double withdrawAmount)
    {
        ...
        if (withdrawAmount % bank.CASH_WITHDRAWAL_MULTIPLE != 0) {
            ...
            Bank.creditBalanceList.put(cardNumber,Bank.creditBalanceList.get(cardNumber)-withdrawAmount-charges);
            if (Bank.creditBalanceList.get(cardNumber) > withdrawAmount && Bank.creditBalanceList.get(cardNumber) > bank.MINIMUM_BALANCE) {
    ...
    {
        Bank bank = new Bank();
        if (swipes < Bank.creditBalanceList.get(cardNumber) && (Bank.creditBalanceList.get(cardNumber) - swipes) > bank.MINIMUM_BALANCE) {
            double cashBack;
            cashBack = (swipes * bank.CASHBACK);
            Bank.creditBalanceList.put(cardNumber, Bank.creditBalanceList.get(cardNumber)- swipes + cashBack);
